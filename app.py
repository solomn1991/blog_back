...
def add_passage():
    ...
    passage_service.add_passage(user_id,title,content)
    result = {"add_passage_success":True,"message":"添加成功"}
    return json.dumps(result)
...
def update_passage():
    ...
    content = request.json.get("content");
    passage_service = PassageService()
    r = passage_service.update_passage(passage_id,title,content)
    result = {"update_success":True,"message":False}
    return json.dumps(result)
